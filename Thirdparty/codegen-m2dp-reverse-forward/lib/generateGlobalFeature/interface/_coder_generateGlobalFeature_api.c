...
{
  real_T (*featureForward)[192];
  real_T (*featureReverse)[192];
  emxArray_uint8_T *img;
  emxArray_real_T *pointCloudForward;
  emxArray_real_T *pointCloudOpposite;
  ...
  featureForward = (real_T (*)[192])mxMalloc(sizeof(real_T [192]));
  featureReverse = (real_T (*)[192])mxMalloc(sizeof(real_T [192]));
  emlrtHeapReferenceStackEnterFcnR2012b(&st);
  emxInit_uint8_T(&st, &img, 2, true);
  emxInit_real_T(&st, &pointCloudForward, 2, true);
  emxInit_real_T(&st, &pointCloudOpposite, 2, true);
  ...
  generateGlobalFeature(img, max_selected_distance, range_resolution,
                        max_distance, *featureForward, pointCloudForward,
                        *featureReverse, pointCloudOpposite);

  /* Marshall function outputs */
  plhs[0] = emlrt_marshallOut(*featureForward);
  emxFree_uint8_T(&img);
  if (nlhs > 1) {
    pointCloudForward->canFreeData = false;
    plhs[1] = b_emlrt_marshallOut(pointCloudForward);
  }

  emxFree_real_T(&pointCloudForward);
  if (nlhs > 2) {
    plhs[2] = emlrt_marshallOut(*featureReverse);
  }

  if (nlhs > 3) {
    pointCloudOpposite->canFreeData = false;
    plhs[3] = b_emlrt_marshallOut(pointCloudOpposite);
  }

  emxFree_real_T(&pointCloudOpposite);
