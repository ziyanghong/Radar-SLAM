...
static void GetSignatureMatrix(const emxArray_real_T *data, double maxRho,
  double A[8192]);
static void PCARotationInvariant(emxArray_real_T *data);
...
void M2DP(emxArray_real_T *data, double desM2DP[192], double A[8192])
{
  emxArray_real_T *y;
  int nx;
  int vstride;
  ...
  /*  numQ = 1; */
  /*  rotation invariant */
  PCARotationInvariant(data);

  /*  Azimuthe list */
  /*  Elevation list */
  /*  get the farthest point distance */
  nx = y->size[0] * y->size[1];
  y->size[0] = static_cast<short>(data->size[0]);
  y->size[1] = 3;
  emxEnsureCapacity_real_T(y, nx);
  nx = static_cast<short>(data->size[0]) * 3;
  for (vstride = 0; vstride < nx; vstride++) {
    y->data[vstride] = data->data[vstride] * data->data[vstride];
  }
  ...
  emxEnsureCapacity_real_T(rho2, nx);
  nx = 2 * y->size[0];
  for (n = 0; n < vstride; n++) {
  ...
  if (rho2->size[0] <= 2) {
    ...
    if (nx == 0) {
      ...
      nx++;
      for (vstride = nx; vstride <= n; vstride++) {
  ...
  GetSignatureMatrix(data, maxRho, A);

  /*  run SVD on A and use [u1,v1] as the final output */
  svd(A, u, s, v);
  std::memcpy(&desM2DP[0], &u[0], 64U * sizeof(double));
  std::memcpy(&desM2DP[64], &v[0], 128U * sizeof(double));
}
