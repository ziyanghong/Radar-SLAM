...
LogSensorStream m_sensorReferenceOpposite(NULL,NULL);
std::vector< std::vector<InterestPoint *> > m_pointsReferenceOpposite;
cv::Mat matches;
...
//     cairo_set_line_width(cairoOut, 1./(2.*scaleFactor));
	...
	std::cout << "Interest points number: " << m_pointsReference[position].size() << std::endl;
	...
	for(unsigned int i = 0; i < m_pointsReference.size(); i++){
		...
		matches.at<uchar>(position,i) = correspondences.size(); // Ziyang
		if(correspondences.size() >= corresp) {
			// std::cout << "correspondences points number: " << correspondences.size() << std::endl;
			...
			cairo_move_to(cairoOut, 0.*drawScale, -3.0*drawScale);
			cairo_line_to(cairoOut, 6.0*drawScale, 0.*drawScale);
			cairo_line_to(cairoOut, 0.*drawScale, 3.0*drawScale);
			cairo_close_path(cairoOut);
			cairo_fill(cairoOut);
			cairo_set_matrix(cairoOut, &m);
		}

		// OrientedPoint2D transformOpposite;		
		// std::vector< std::pair<InterestPoint*, InterestPoint* > > correspondencesOpposite;
		// double resultOpposite = m_ransac->matchSets(m_pointsReferenceOpposite[i], pointsLocal, transformOpposite, correspondencesOpposite);	
		// if(correspondencesOpposite.size() >= corresp) {
		// 	cairo_matrix_t m;
		// 	cairo_get_matrix(cairoOut, &m);
		// 	cairo_translate(cairoOut, transformOpposite.x, transformOpposite.y);
		// 	cairo_rotate(cairoOut, transformOpposite.theta);
...
void detectOppositeLog(){
	unsigned int i = 0;
	unsigned int position = m_sensorReferenceOpposite.tell();
	m_sensorReferenceOpposite.seek(0,END);
	unsigned int last = m_sensorReferenceOpposite.tell();
	m_sensorReferenceOpposite.seek(0);

	std::string bar(50, ' ');
	bar[0] = '#';
	unsigned int progress = 0;

	while(!m_sensorReferenceOpposite.end()){
		unsigned int currentProgress = (m_sensorReferenceOpposite.tell()*100)/last;
		if (progress < currentProgress){
			progress = currentProgress;
			bar[progress/2] = '#';
			// std::cout << "\rDetecting points  [" << bar << "] " << (m_sensorReferenceOpposite.tell()*100)/last << "%" << std::flush;
		}
		const LaserReading* lreadReference = dynamic_cast<const LaserReading*>(m_sensorReferenceOpposite.next());
		if (lreadReference){
			m_detector->detect(*lreadReference, m_pointsReferenceOpposite[i]);
			i++;
		}
	}
	m_sensorReferenceOpposite.seek(position);
	std::cout << " done." << std::endl;
}
...
void describeOppositeLog(){
	...
	while(!m_sensorReferenceOpposite.end()){
		...
		if (progress < currentProgress){
			progress = currentProgress;
			bar[progress/2] = '#';
			// std::cout << "\rDescribing points  [" << bar << "] " << (m_sensorReferenceOpposite.tell()*100)/last << "%" << std::flush;
		}
		const LaserReading* lreadReference = dynamic_cast<const LaserReading*>(m_sensorReferenceOpposite.next());
		if (lreadReference){
			for(unsigned int j = 0; j < m_pointsReferenceOpposite[i].size(); j++){
				m_pointsReferenceOpposite[i][j]->setDescriptor(m_descriptor->describe(*m_pointsReferenceOpposite[i][j], *lreadReference));
			}
			i++;
		}
	}
...
int main(int argc, char **argv){
	...
	double baseSigma = 0.2;
	// double baseSigma = 0.4; // Ziyang 
	double sigmaStep = 1.4, minPeak = 0.34, minPeakDistance = 0.001, success = 0.95, inlier = 0.4, matchingThreshold = 0.4;
	bool useMaxRange = false;
	int maxFrame = 1; // Maxumum accumulated frames
	int downSampleScan = 1; // downsample ratio from scans
	double distanceScaleFactor = 2.; // Variable defined for CurvatureDetector.h
	...
	while(i < argc){
		if(strncmp("-filename", argv[i], sizeof("-filename")) == 0 ){
			...
			i++;
		} else if(strncmp("-drawScale", argv[i], sizeof("-drawScale")) == 0 ){
			drawScale = strtod(argv[++i], NULL);			
			i++;
		} else if(strncmp("-maxFrame", argv[i], sizeof("-maxFrame")) == 0 ){
			maxFrame = atoi(argv[++i]);
			i++;
		} else if(strncmp("-downSampleScan", argv[i], sizeof("-downSampleScan")) == 0 ){
			downSampleScan = atoi(argv[++i]);
			i++;
		} else if(strncmp("-distanceScaleFactor", argv[i], sizeof("-distanceScaleFactor")) == 0 ){
			distanceScaleFactor = strtod(argv[++i], NULL);
			i++;									
		// } else if(strncmp("-probabilityOfFalseAlarm", argv[i], sizeof("-probabilityOfFalseAlarm")) == 0 ){
		// 	probabilityOfFalseAlarm = strtod(argv[++i], NULL);			
		// 	i++;
		} else {
			i++;			
		}
	...
	reader.directionDescriptor = 0;
	reader.m_MAX_FRAME = maxFrame;
	reader.m_DOWN_SAMPLE_SCAN = downSampleScan;
	...
	m_detector->m_distanceScaleFactor = distanceScaleFactor;
	...
	detectLog();
	describeLog();
	matches = cv::Mat::zeros(cv::Size(m_pointsReference.size(), m_pointsReference.size()), CV_8U);

	//---------------------------------- Load and compute opposite points descriptor----------------------------------------
	// CarmenLogWriter writerOpposite;
	// CarmenLogReader readerOpposite;
	// readerOpposite.directionDescriptor = 1;		
	// m_sensorReferenceOpposite = LogSensorStream(&readerOpposite, &writerOpposite);
	// m_sensorReferenceOpposite.load(filename);
	// m_sensorReferenceOpposite.seek(0,END);
	// m_sensorReferenceOpposite.seek(0,BEGIN);
	// m_pointsReferenceOpposite.resize(end + 1);
	// detectOppositeLog();
	// describeOppositeLog();
	//--------------------------------- Load and compute opposite points descriptor-----------------------------------------
...
configFile << outDir.str() << "configuration.txt";
std::string outpath = configFile.str();
std::ofstream txtfile(outpath.c_str());
if (txtfile.is_open())
{
	txtfile 
	...
	<< "detectorType:" << detectorType << std::endl
	<< "descriptorType:" << descriptorType << std::endl   
	<< "distanceType:" << distanceType << std::endl
	<< "baseSigma:" << baseSigma << std::endl
	<< "sigmaStep:" << sigmaStep << std::endl
	<< "minPeak:" << minPeak << std::endl
	<< "minPeakDistance:" << minPeakDistance << std::endl
	<< "acceptanceSigma:" << acceptanceSigma << std::endl
	<< "success:" << success << std::endl
	<< "inlier:" << inlier << std::endl
	<< "matchingThreshold:" << matchingThreshold << std::endl
	<< "strategy:" << strategy << std::endl
	<< "maxFrame:" << maxFrame << std::endl
	<< "downSampleScan:" << downSampleScan << std::endl
	<< "distanceScaleFactor:" << distanceScaleFactor << std::endl;
...
///////////////////////////////////////////////// Output matches matrix /////////////////////////////////////////////////////
std::stringstream matchesPath;
matchesPath << outDir.str() << "matches.txt";
std::string matches_file = matchesPath.str();
cv::FileStorage file(matches_file.c_str(), cv::FileStorage::WRITE);
// Write to file!
file << "confusion" << matches;;
///////////////////////////////////////////////// Output matches matrix /////////////////////////////////////////////////////
