...
void FLIRTDemo::processReading(const AbstractReading* _read){
    ...
    if (lread){
        ...
        // std::cout << "points size in Demo: " << points.size() << std::endl; // Ziyang
        ...
        Detector* detector = m_chooser->getCurrentDetector();
        detector->m_distanceScaleFactor = distanceScaleFactor; // Ziyang
		detector->detect(*lread, m_points, signal, smooth, diff, idx); 
        // std::cout << "m_points size in Demo: " << m_points.size() << std::endl; // Ziyang
        // std::cout << "smooth size in Demo: " << smooth.size() << std::endl; // Ziyang
        ...
        // std::cout << "m_colors size: " << m_colors.size() << std::endl; // Ziyang
		for(unsigned int i = 0; i < m_points.size(); i++){
