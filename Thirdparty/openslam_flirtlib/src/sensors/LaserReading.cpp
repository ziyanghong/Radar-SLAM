...
void LaserReading::setLaserPose(const OrientedPoint2D& _pose){
    m_laserPose = _pose;
    // std::cout << "x: " << m_laserPose.x << ", y: "<< m_laserPose.y << ", theta: " << m_laserPose.theta << std::endl;
    computeWorldCartesian();
}

// void LaserReading::setRadarPose(const OrientedPoint2D& _pose){
//     m_laserPose = _pose;
//     m_worldCartesian.resize(m_cartesian.size());
//     //std::cout<< "m_worldCartesian size： "<< m_worldCartesian.size() << std::endl;
//     m_worldCartesianNoMax.clear();
//     for(unsigned int i = 0; i < m_cartesian.size(); i++){
//         // m_worldCartesian[i].x = cos(m_laserPose.theta)*m_cartesian[i].x + -sin(m_laserPose.theta)*m_cartesian[i].y + m_laserPose.x;
//         // m_worldCartesian[i].y = sin(m_laserPose.theta)*m_cartesian[i].x +  cos(m_laserPose.theta)*m_cartesian[i].y + m_laserPose.y;
...
void LaserReading::setLocalCartesian(const std::vector<double> xs, const std::vector<double> ys){
    m_cartesian.resize(xs.size());
    //std::cout<< "m_cartesian size： "<< m_cartesian.size() << std::endl;

    m_cartesianNoMax.clear();
    for(unsigned int i= 0 ; i < xs.size(); i++){
        m_cartesian[i].x = xs[i];
        m_cartesian[i].y = ys[i];
    }
}
