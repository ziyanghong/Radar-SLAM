...
void CurvatureDetector::detect(const Graph& graph, const std::vector<Point2D>& graphPoints, 
std::vector< std::vector<Point2D> >& operatorA, std::vector< std::vector<double> >& signalDiff, 
				       std::vector< std::vector<unsigned int> >& indexes) const
{
    ...
    bool bPath = boost::johnson_all_pairs_shortest_paths(graph, distances);
    // bool bPath = boost::floyd_warshall_all_pairs_shortest_paths(graph, distances);
    // std::cout << "m_scales.size(): " << m_scales.size() << std::endl;
	// std::cout<< "Found path: " << bPath << std::endl;
    // std::cout << "vertexNumber: " << vertexNumber << std::endl;
  	// std::cout <<"rows of distances: " << distances.size() << " cols: " << distances[0].size() << std::endl;
  	// std::cout <<"capacity of distances: " << distances.capacity() << " capacity: " << distances[0].capacity() << std::endl;
    ...
    for(unsigned int i = 0; i < indexes.size(); i++){
		for(unsigned int j = 0; j < indexes[i].size(); j++){
			...
			if(reading.getRho()[pointIndex] >= reading.getMaxRange()){
				// std::cout << "if(reading.getRho()[pointIndex] >= reading.getMaxRange())" << std::endl;
				continue;
			}
			...
			// }
			// if(exists) {
			// 	std::cout << "exists = exists || (fabs(pose.x - point[k]->getPosition().x) <= 0.2 &&  fabs(pose.y - point[k]->getPosition().y) <= 0.2);"<<std::endl;
			// 	continue;
			// }

			// double distance = 2. * m_scales[i];
			double distance = m_distanceScaleFactor * m_scales[i]; // Ziyang
			// std::cout << "m_distanceScaleFactor * m_scales[i]: " << distance << std::endl;
			...
			if(hypot(diffStart.x, diffStart.y) < distance || hypot(diffEnd.x, diffEnd.y) < distance){
				// std::cout << "if(hypot(diffStart.x, diffStart.y) < distance || hypot(diffEnd.x, diffEnd.y) < distance)" << std::endl;
				continue;
			}
...
// 	    pose.theta = normAngle(param.alpha, - M_PI);    
		...
		//  InterestPoint *interest = new InterestPoint(pose, m_scales[i]);
			// std::cout << "point theta: " << pose.theta << std::endl; // Ziyang
    ...
    std::cout << "InterestPoint size " << point.size() << std::endl; // Ziyang
    return point.size();
