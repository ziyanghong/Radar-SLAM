...
class CarmenLogReader: public LogReader{
	...
	/** For descriptor direction. */
	int directionDescriptor;
    protected:
	...
	/** Parse the raw radar cartesian image (RADAR). */	
	LaserReading* parseRadar(std::istream& _stream) const;

public:
	// Ziyang
	int m_MAX_FRAME;
	int m_DOWN_SAMPLE_SCAN;
};
