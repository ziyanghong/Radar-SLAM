...
KDNode::KDNode(const std::vector< double > &pt, const size_t &idx_, const KDNodePtr &left_,
...
KDNode::operator std::vector< double >() { return x; }
...
KDNodePtr KDTree::nearest_(   //
    ...
    std::vector< double > branch_pt(*branch);
...
// default caller
KDNodePtr KDTree::nearest_(const std::vector< double > &pt) {
    size_t level = 0;
    // KDNodePtr best = branch;
    double branch_dist = dist2(std::vector< double >(*root), pt);
...
std::vector< double > KDTree::nearest_point(const std::vector< double > &pt) {
    return std::vector< double >(*nearest_(pt));
};
size_t KDTree::nearest_index(const std::vector< double > &pt) {
    return size_t(*nearest_(pt));
};

pointIndex KDTree::nearest_pointIndex(const std::vector< double > &pt) {
    KDNodePtr Nearest = nearest_(pt);
    return pointIndex(std::vector< double >(*Nearest), size_t(*Nearest));
}
...
pointIndexArr KDTree::neighborhood_(  //
    ...
    d = dist2(std::vector< double >(*branch), pt);
    dx = std::vector< double >(*branch).at(level) - pt.at(level);
    dx2 = dx * dx;
